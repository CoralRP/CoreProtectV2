...
class ContainerTransactionProcess {
    ...
    static void process(PreparedStatement preparedStmtContainer, PreparedStatement preparedStmtItems, int batchCount, int processId, int id, Material type, int forceData, String user, Object object, boolean glove) {
            ...
            if (inventories.get(id) != null) {
                ...
                if (ConfigHandler.loggingChest.get(loggingChestId) != null) {
                    ...
                    if (current_chest == forceData || force_size > 0) { // This prevents client side chest sorting mods from messing things up.
                        ContainerLogger.log(preparedStmtContainer, preparedStmtItems, batchCount, user, type, inventory, location, glove);
